...
fun sim_files_cp ghc tmp = 
  ...
  " in writeln \"Compiling animation...\"; bash (\"cd \" ^ Path.implode path' ^ \"; " ^ ghc ^ " -Wno-unrecognised-warning-flags -Wno-x-partial Animation >> /dev/null\") ; copy_dir path' (Path.explode \"" ^ tmp ^ "\") end)";
